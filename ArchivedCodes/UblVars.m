classdef UBLDef
    ...
    methods
        ...
        function obj = UBLModel(obj,sensHeat,refSite,rural,forc,...
               ...
               if gt(numel(obj),1) % more than one urban area
                  ...
                  for i = 1:numel(first)
                      [obj(first(i)).ublTemp,obj(first(i)).ublTempdx] = NightForc(obj(first(i)).ublTempdx,...
                          ...
                          refSite,obj(first(i)).ublEmis,obj(first(i)).atmTemp,obj(first(i)).surfTemp);
                      firstTemp = firstTemp + obj(first(i)).ublTemp;
                  end
                  firstTemp = firstTemp/numel(first);
                  secondTemp = 0.;
                  for i = 1:numel(second)
                      [obj(second(i)).ublTemp,obj(second(i)).ublTempdx] = NightForcUrb(obj(second(i)).ublTempdx,...
                          ...
                          refSite,obj(second(i)).ublEmis,obj(second(i)).atmTemp,obj(second(i)).surfTemp);     
                      secondTemp = secondTemp + obj(second(i)).ublTemp;
                  end
                  secondTemp = secondTemp/numel(second);
                  if ne(numel(third),0)
                        for i = 1:numel(third) 
                          ...
                          refSite,obj(third(i)).ublEmis,obj(third(i)).atmTemp,obj(third(i)).surfTemp);
                        end
                  end
               else % One urban area
                  [obj.ublTemp,obj.ublTempdx] = NightForc(obj.ublTempdx,...
                      ...
                      refSite,obj.ublEmis,obj.atmTemp,obj.surfTemp);
               end
            end
        end
    end
